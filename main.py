...
for iteration in range(1000):
    ...
    mag_A = base_mag_A
    mag_S = base_mag_S
    reward = base_reward + floor(iteration / 10)
    print(reward)
    ...
    P = FLMDP.random_P(mag_S=mag_S, mag_A=mag_A, l=l, mean_reward=reward)
    lmdp = FLMDP(mag_S=mag_S, mag_A=mag_A, P=P, P0=P0, l=l)
    ...
    s_b, a_b, r_b = lmdp.simulate(pi=pi_b, T=T, m=m)
    s_e, a_e, r_e = lmdp.simulate(pi=pi_e, T=T, m=m)

    # Naive Monte-Carlo Policy Estimator
    hat_b = naive_approx(states=s_b, actions=a_b, rewards=r_b, l=l)
...
pickle.dump(rmses_naive, open('rmses_native_reward.pickle', 'wb'))
pickle.dump(rmses_sc, open('rmses_sc_reward.pickle', 'wb'))
pickle.dump(perfect_returns, open('perfect_returns_reward.pickle', 'wb'))
pickle.dump(naive_returns, open('naive_returns_reward.pickle', 'wb'))
pickle.dump(sc_returns, open('sc_returns_reward.pickle', 'wb'))
pickle.dump(true_returns, open('true_returns_reward.pickle', 'wb'))
